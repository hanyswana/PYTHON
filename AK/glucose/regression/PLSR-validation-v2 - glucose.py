...
MODEL_FILE = 'Lablink_1k_glucose_RAW_PLSR_best_model.joblib' # <-- pick one model from model/2nd
VAL_CSV    = f'{BASE_DIR}/dataset/dataset-1k-glucose/ori/csv/Lablink_1k_glucose_RAW.csv'  # your validation CSV
OUT_DIR    = f'{BASE_DIR}/validation-output-LABLINK/1st-best'         # outputs here
...
csv_path = os.path.join(OUT_DIR, os.path.splitext(os.path.basename(VAL_CSV))[0] + '_actual_vs_pred.csv')
...
plt.title(f'Glucose LABLINK Validation - PLSR\nr={pearson_r:.3f}  R²={r2:.3f}  RMSE={rmse:.2f}\nISO Within: {iso_within_pct:.1f}%')
plt.xlabel('Actual Glucose (mmol/L)')
plt.ylabel('Predicted Glucose (mmol/L)')
plt.grid(True, alpha=0.3)
plt.tight_layout()
plot_path = os.path.join(OUT_DIR, os.path.splitext(os.path.basename(VAL_CSV))[0] + '_actual_vs_pred_plot.png')
plt.savefig(plot_path, dpi=220)
plt.close()

# --- Save metrics summary CSV ---
metrics_path = os.path.join(OUT_DIR, os.path.splitext(os.path.basename(VAL_CSV))[0] + '_metrics.csv')
pd.DataFrame([{
    'Model File': MODEL_FILE,
    'Validation CSV': os.path.basename(VAL_CSV),
    'RMSE': rmse,
    'R^2': r2,
    'Pearson r': pearson_r,
