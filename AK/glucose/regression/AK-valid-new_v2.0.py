...
# Define file paths
csv = 'USIM-data-2026-ALL-csv-completed'
dir = '/home/apc-3/PycharmProjects/PythonProjectAK/validation-1k-glucose'
data_file = f'{dir}/dataset/USIM/{csv}.csv'

# TensorFlow model paths and output == 4th model == FINALIZE ==
mod = 'r2_0.33_87_'
tf_model = f'Lablink_1k_glucose_RAW_top_10_Norm_Euc_batch16_{mod}'
model_dir = "9th_r2_0.33_87_"
tf_model_dir = f'{dir}/model/model-NEW-FLOW/{model_dir}/{tf_model}'
output_file = f'{dir}/result/result-USIM/model-NEW-FLOW/{model_dir}/M1/without-CT/completed'
tf_output_file_19 = f'{output_file}/RESULT_19_tf_{csv}_pp_model_{mod}.csv'

# TFLite & TFLite model paths and output
tflite_model = '9th_Lablink_1k_glucose_RAW_top_10_Norm_Euc_batch16_r2_0.33_87_.tflite'
tflite_model_dir = f'{dir}/model/model-NEW-FLOW/{model_dir}/{tflite_model}'
...
# tf_model_dir = f'{dir}/model/model-NEW-FLOW/{model_dir}/{tf_model}'
# output_file = f'{dir}/result/result-ALTY/model-NEW-FLOW/{model_dir}/new-pp-cw-150'
# tf_output_file_19 = f'{output_file}/RESULT_19_tf_{csv}_pp_model_{mod}.csv'
#
# # TFLite & TFLite model paths and output
...
# # TensorFlow model paths and output ===== PINTARAI =====
# mod = 'max_trials_15'
# tf_model = f'Lablink_1k_glucose_Norm_Manh_SNV_2nd_Deriv_{mod}'
# model_dir = "model-1k-gl-PINTARAI-4th"
# tf_model_dir = f'{dir}/model/model-PINTARAI/{model_dir}/{tf_model}'
# output_file = f'{dir}/result/result-ALTY/model-NEW-FLOW/{model_dir}/new-pp'
# tf_output_file_19 = f'{output_file}/RESULT_19_tf_{csv}_pp_model_{mod}.csv'
#
# # TFLite & TFLite model paths and output
# # tflite_model = f'6th_LL_1k_gl_model_{mod}.tflite'
# tflite_model = 'Lablink_1k_glucose_Norm_Manh_SNV_2nd_Deriv_max_trials_15.tflite'
# tflite_model_dir = f'{dir}/model/model-NEW-FLOW/{model_dir}/{tflite_model}'
...
# === SELECT THE 10 TRAINING BANDS FIRST (from RAW) ===
selected_wavelengths = ['555 nm', '560 nm', '585 nm', '630 nm', '645 nm', '680 nm', '810 nm', '860 nm', '900 nm', '940 nm']  # RAW 10 bands used by the model
spectral_data = all_spectral_data[selected_wavelengths].copy()
...
snv_data = pd.DataFrame(snv_array, columns=norm_euc_data.columns, index=norm_euc_data.index)
...
# --- TensorFlow Model Inference (W/O custom weight) ---
...
first_two_columns = df.iloc[:, :3]  # Keep the first 3 columns
actual_glucose_columns = df.loc[:, "actual-glucose (mmol/L)"]  # Keep the actual glucose columns
pred_v1_glucose_columns = df.loc[:, "v1-predicted-glucose (mmol/L)"]  # Keep the pred v1 glucose columns
...
combined_df = pd.concat(
    [
        ...
        actual_glucose_columns.reset_index(drop=True),
        pred_v1_glucose_columns.reset_index(drop=True),
        pd.DataFrame({
...
y_true = df.loc[:, "actual-glucose (mmol/L)"].astype(float).values  # adjust if your GL column moves
print(y_true)
...
plt.title(
    f'Glucose USIM Validation (W1-W2)\n'
