...
BASE_FOLDER = '/home/admin-3/PycharmProjects/PythonProjectAK-REVA/1k-lablink-L1L2-dropout/1k-hb'
...
# File locations
DATASET_FOLDER = os.path.join(BASE_FOLDER, 'dataset/run/dataset-1k-hb-pls-top10-parquet-5')
...
def train_and_predict(dataset):
    try:
        ...
        result_file = os.path.join(RESULT_FOLDER, f'results-all-datasets-1k-hb.csv')
        hyperparameter_file = os.path.join(HYPERPARAMETER_FOLDER, f'{dataset_name}_hyperparameter.json')
        # tuning_folder = os.path.join(BASE_FOLDER, '1k-hb-all-tuning', f'{dataset_name}-tuning')
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X_train), start=1):
            ...
            cv_mse_scores.append(mean_squared_error(y_val_fold, y_pred))
