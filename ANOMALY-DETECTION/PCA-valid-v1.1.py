#!/usr/bin/env python3
"""
Validate saved PCA anomaly detector (T² + Q) models (v1.1) using ONE matching validation CSV per model.

Matching rule:
- Model tag = folder name under MODEL_DIR_BASE:
    .../PCA/model/v1.1/<tag>/pca_model_best.joblib
...
Outputs:
- Per model:
    .../PCA/validation-output/v1.1/<model_tag>/
        - validation_results.csv
        - validation_summary.txt
        - _DONE.ok
        - error_log.txt (if failed)
- Global summary CSV:
    .../PCA/validation-output/v1.1/summary_all_validations.csv
...
OUT_DIR_BASE = f"{DIR}/PCA/validation-output/v1.1"
...
def model_tag_from_path(model_joblib_path: str) -> str:
    """.../PCA/model/v1.1/<tag>/pca_model_best.joblib -> <tag>"""
...
def find_all_models(model_dir_base: str) -> list[str]:
    pattern = os.path.join(model_dir_base, "*", "pca_model_best.joblib")
...
def validate_one(model_path: str, data_path: str) -> dict:
    ...
    if spectral_cols_saved:
        ...
        if missing:
            raise ValueError(
                "Validation CSV missing spectral columns used in training:\n"
                + "\n".join(missing[:60])
    ...
    if total:
        score_stats = {
            ...
            "p90": float(np.percentile(anomaly_score, 90)),
    ...
    with open(out_summary_txt, "w", encoding="utf-8") as f:
        f.write("PCA Validation (T² + Q) - v1.1 (1 model ↔ 1 dataset)\n")
        f.write("----------------------------------------------------\n")
...
def main():
    ...
    if not model_paths:
        raise FileNotFoundError(f"No models found under: {MODEL_DIR_BASE}/*/pca_model_best.joblib")
    ...
    log(f"[INFO] Found {len(model_paths)} PCA models in {MODEL_DIR_BASE}")
