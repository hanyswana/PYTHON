#!/usr/bin/env python3
"""
Validate saved PyOD Isolation Forest models (v1.1) using ONE matching validation CSV per model.

Matching rule:
- Model tag = folder name under MODEL_DIR_BASE:
    .../IF/model/v1.1/<tag>/iforest_pyod_model_best.joblib
...
Outputs:
- Per model:
    .../IF/validation-output/v1.2/<model_tag>/
        - validation_results.csv
        - validation_summary.txt
        - _DONE.ok
        - error_log.txt (if failed)
- Global summary CSV:
    .../IF/validation-output/v1.2/summary_all_validations.csv
...
OUT_DIR_BASE = f"{DIR}/IF/validation-output/v1.1"
...
def model_tag_from_path(model_joblib_path: str) -> str:
    """.../IF/model/v1.1/<tag>/iforest_pyod_model_best.joblib -> <tag>"""
...
def find_all_models(model_dir_base: str) -> list[str]:
    pattern = os.path.join(model_dir_base, "*", "iforest_pyod_model_best.joblib")
...
def validate_one(model_path: str, data_path: str) -> dict:
    """
    Validate one model on its matched dataset, save outputs, return summary row.
    """
    t0 = time.time()
    ...
    # Predict + score
    y_pred = clf.predict(X_np)           # 0=pass, 1=fail
    ...
    with open(out_summary_txt, "w", encoding="utf-8") as f:
        f.write("Isolation Forest Validation (PyOD) - v1.2 (1 model ↔ 1 dataset)\n")
        f.write("--------------------------------------------------------------\n")
...
def main():
    ...
    if not model_paths:
        raise FileNotFoundError(f"No models found under: {MODEL_DIR_BASE}/*/iforest_pyod_model_best.joblib")
    ...
    log(f"[INFO] Found {len(model_paths)} models in {MODEL_DIR_BASE}")
    ...
    for idx, m in enumerate(model_paths, start=1):
        ...
        if expected_validation_outputs_exist(out_dir):
            ...
            summary_rows.append({
                "model_tag": mtag,
                "model_path": m,
                "matched_dataset_file": valid_map.get(mtag, ""),
