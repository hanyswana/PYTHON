...
UPDATED (debug + global summary):
- Print debug status for each dataset and each stage
- Track success/failure per dataset
- Save 1 global summary CSV for all datasets at OUT_DIR_BASE/summary_all_datasets.csv
"""

import os, sys, joblib, math, glob, time, json, traceback
import numpy as np
import pandas as pd
...
OUT_DIR_BASE = f"{DIR}/IF/result/v1.1"
MODEL_DIR_BASE = f"{DIR}/IF/model/v1.1"
...
# ----------------------------
def log(msg: str):
    """Timestamped logger that won't crash if stdout pipe breaks (e.g., PyCharm crash)."""
    ts = time.strftime("%Y-%m-%d %H:%M:%S")
    try:
        print(f"[{ts}] {msg}", flush=True)
    except BrokenPipeError:
        # Avoid killing the training if IDE console closes
        pass
...
def expected_outputs_exist(out_dir: str, model_dir: str, algo: str) -> bool:
    """
    ...
    # Key files to consider as "completed"
    common = [
    ...
    if algo.upper() == "IF":
        model_files = [
            os.path.join(model_dir, "iforest_pyod_model_best.joblib"),
            os.path.join(model_dir, "iforest_pyod_model_best.h"),
        ]
    else:  # MD
        model_files = [
...
def heuristic_eval_one_split(X_np: np.ndarray, params: dict, seed: int):
    ...
    y_val = clf.predict(X_val)  # 0=inlier, 1=outlier
...
def run_one_dataset(csv_file: str) -> dict:
    """
    Train for one dataset and return a dict row for the global summary CSV.
    This function raises exceptions; caller handles status.
    """
    t0 = time.time()
    ...
    # ----- Stage 2: Build X, clean NaNs/Infs
    ...
    export_pyod_iforest_joblib_to_header(
        best_model_path,
        best_header_path,
        header_guard=f"IFOREST_PYOD_MODEL_BEST_H_{tag.upper()}".replace("-", "_"),
    )
...
def main():
    ...
    # Global summary rows (success + fail)
    summary_rows = []
    ...
    for i, csv_file in enumerate(csv_files, start=1):
        ...
        # ✅ ADD THIS BLOCK (HERE)
        # ===========================
        if expected_outputs_exist(out_dir, model_dir, algo="IF"):  # use "MD" in MD script
            ...
            # Update global summary immediately
            pd.DataFrame(summary_rows).to_csv(
                os.path.join(OUT_DIR_BASE, "summary_all_datasets.csv"),
                index=False
            )
            continue
        try:
        ...
        # Save running summary after each dataset (so you never lose progress)
    ...
    log(f"Global summary saved at: {os.path.join(OUT_DIR_BASE, 'summary_all_datasets.csv')}")
