#!/usr/bin/env python3
"""
Validate saved PyOD Isolation Forest (best joblib) on NEW dataset.

- Model:   /home/apc-3/PycharmProjects/PythonProjectAD/IF/model/v1.1/iforest_pyod_model_best.joblib
- Dataset: /home/apc-3/PycharmProjects/PythonProjectAD/dataset-glucose/raw/Lablink_330_validation.csv

Outputs:
- /home/apc-3/PycharmProjects/PythonProjectAD/IF/validation-output/v1.1/validation_results.csv
- /home/apc-3/PycharmProjects/PythonProjectAD/IF/validation-output/v1.1/validation_summary.txt
"""
...
MODEL_PATH = f"{DIR}/IF/model/v1.0/iforest_pyod_model_best.joblib"
DATA_PATH  = f"{DIR}/dataset-glucose/raw/Lablink_330_validation.csv"

OUT_DIR = f"{DIR}/IF/validation-output/v1.1"
...
def main():
    ...
    #    If your file uses different names, change them here.
    meta_cols = []
    for c in ["Sample", "glucose"]:
        if c in df.columns:
            meta_cols.append(c)

    # If missing, still proceed but without those columns
    ...
    if spectral_cols_saved is not None and len(spectral_cols_saved) > 0:
        ...
        # fallback: use current spectral range order
    ...
    with open(OUT_SUMMARY_TXT, "w", encoding="utf-8") as f:
        f.write("Isolation Forest Validation (PyOD)\n")
        f.write("---------------------------------\n")
