#!/usr/bin/env python3
"""
Validate saved Mahalanobis Distance (MD) models (v1.1) using ONE matching validation CSV per model.

Matching rule:
- Model tag = folder name under MODEL_DIR_BASE:
    .../MD/model/v1.1/<tag>/md_model_best.joblib
- Validation CSV filename must be:
    Lablink_330_validation_<tag>.csv
  inside VALID_CSV_DIR

Outputs (same structure as IF-valid v1.1, but for MD):
- Per model:
    .../MD/validation-output/v1.1/<model_tag>/
        - validation_results.csv
        - validation_summary.txt
        - _DONE.ok
        - error_log.txt (if failed)
- Global summary CSV:
    .../MD/validation-output/v1.1/summary_all_validations.csv

Resume/Skip:
- If _DONE.ok exists (or outputs exist), it will skip that model.
"""
...
MODEL_DIR_BASE = f"{DIR}/MD/model/v1.1"
VALID_CSV_DIR  = f"{DIR}/dataset-glucose/24/validate/pp"

OUT_DIR_BASE = f"{DIR}/MD/validation-output/v1.1"
...
# ----------------------------
def log(msg: str):
    """Robust logger (won't crash if PyCharm/pipe closes)."""
...
def model_tag_from_path(model_joblib_path: str) -> str:
    """.../MD/model/v1.1/<tag>/md_model_best.joblib -> <tag>"""
...
def find_all_models(model_dir_base: str) -> list[str]:
    pattern = os.path.join(model_dir_base, "*", "md_model_best.joblib")
...
def validate_one(model_path: str, data_path: str) -> dict:
    """
    Validate one MD model on its matched dataset, save outputs, return summary row.
    """
    t0 = time.time()
    ...
    # Load model bundle (saved by MD training v1.1)
    bundle = joblib.load(model_path)

    mean = bundle.get("mean", None)
    inv_cov = bundle.get("inv_cov", None)
    threshold = bundle.get("threshold", None)
    spectral_cols_saved = bundle.get("spectral_columns", None)
    config = bundle.get("config", {})

    if mean is None or inv_cov is None or threshold is None:
        raise ValueError("Joblib bundle missing required keys: mean, inv_cov, threshold (plus spectral_columns, config).")

    mean = np.asarray(mean, dtype=np.float32)
    inv_cov = np.asarray(inv_cov, dtype=np.float32)
    ...
    # Score + predict
    scores = mahalanobis_d2(X_np, mean, inv_cov)              # md^2
    y_pred = (scores > threshold).astype(np.int32)            # 0=pass, 1=fail
    ...
    with open(out_summary_txt, "w", encoding="utf-8") as f:
        f.write("Mahalanobis Distance Validation (MD^2) - v1.1 (1 model ↔ 1 dataset)\n")
        f.write("----------------------------------------------------------------\n")
        ...
        f.write("Decision:\n")
        f.write("  fail if md^2 > threshold\n")
        f.write(f"  threshold_md2={threshold:.10f}\n\n")
        ...
        if score_stats:
            f.write("Score stats (md^2, higher = more anomalous):\n")
...
def main():
    ...
    if not model_paths:
        raise FileNotFoundError(f"No models found under: {MODEL_DIR_BASE}/*/md_model_best.joblib")
    ...
    log(f"[INFO] Found {len(model_paths)} MD models in {MODEL_DIR_BASE}")
