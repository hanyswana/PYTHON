#!/usr/bin/env python3
"""
Validate saved PyOD One-Class SVM (OCSVM) models (v1.1) using ONE matching validation CSV per model.

Matching rule:
- Model tag = folder name under MODEL_DIR_BASE:
    .../SVM/model/v1.1/<tag>/ocsvm_pyod_model_best.joblib
...
Outputs:
- Per model:
    .../SVM/validation-output/v1.1/<model_tag>/
        - validation_results.csv
        - validation_summary.txt
        - _DONE.ok
        - error_log.txt (if failed)
- Global summary CSV:
    .../SVM/validation-output/v1.1/summary_all_validations.csv
...
OUT_DIR_BASE = f"{DIR}/SVM/validation-output/v1.1"
...
def model_tag_from_path(model_joblib_path: str) -> str:
    """.../SVM/model/v1.1/<tag>/ocsvm_pyod_model_best.joblib -> <tag>"""
...
def find_all_models(model_dir_base: str) -> list[str]:
    pattern = os.path.join(model_dir_base, "*", "ocsvm_pyod_model_best.joblib")
...
def validate_one(model_path: str, data_path: str) -> dict:
    """
    Validate one OCSVM model on its matched dataset, save outputs, return summary row.
    """
    t0 = time.time()
    ...
    # Load model bundle (same schema as SVM training v1.1)
    ...
    with open(out_summary_txt, "w", encoding="utf-8") as f:
        f.write("One-Class SVM Validation (PyOD OCSVM) - v1.1 (1 model ↔ 1 dataset)\n")
        f.write("--------------------------------------------------------------\n")
...
def main():
    ...
    if not model_paths:
        raise FileNotFoundError(f"No models found under: {MODEL_DIR_BASE}/*/ocsvm_pyod_model_best.joblib")
    ...
    log(f"[INFO] Found {len(model_paths)} SVM models in {MODEL_DIR_BASE}")
